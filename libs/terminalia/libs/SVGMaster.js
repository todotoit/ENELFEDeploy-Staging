...
SVGMASTER.SvgMaster = function SvgMaster() {
    ...
    function loadSvg(svgFile, id, callback){
		d3.xml(svgFile, "image/svg+xml", function (error, data) {
	        if (error) {
	        	...
	        	var svgDocument = SVGNormalize(data);       
	        	var body = d3.select('body');
	        	body.node().appendChild(svgDocument.documentElement);
	        	var svg = body.select('#'+id);
	        	svg.style('display', 'none');
	        	// clean svg from height and width
    ...
    function SVGNormalize(data) {
	    var doctype = data.doctype;
	    // http://stackoverflow.com/a/10162353/715002
	    var svgDoctype = '<!DOCTYPE ' +
	                doctype.name +
	                (doctype.publicId ? ' PUBLIC "' + doctype.publicId + '"' : '') +
	                (!doctype.publicId && doctype.systemId ? ' SYSTEM' : '') +
	                (doctype.systemId ? ' "' + doctype.systemId + '"' : '') +
	                '>';
	    // http://stackoverflow.com/a/26905999/715002
	    var svgContent = data.documentElement.outerHTML.replace(/(font-size:.*);/g, '$1px');
